...
static void ZfsArcMeter_display(const Object* cast, RichString* out) {
   ...
   if (this->values[5] > 0) {
      ...
      RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " MFU:");
      RichString_appendAscii(out, CRT_getAttrs(ZFS_MFU), buffer);
      Meter_humanUnit(buffer, this->values[1], sizeof(buffer));
      RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " MRU:");
      RichString_appendAscii(out, CRT_getAttrs(ZFS_MRU), buffer);
      Meter_humanUnit(buffer, this->values[2], sizeof(buffer));
      RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " Anon:");
      RichString_appendAscii(out, CRT_getAttrs(ZFS_ANON), buffer);
      Meter_humanUnit(buffer, this->values[3], sizeof(buffer));
      RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " Hdr:");
      RichString_appendAscii(out, CRT_getAttrs(ZFS_HEADER), buffer);
      Meter_humanUnit(buffer, this->values[4], sizeof(buffer));
      RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " Oth:");
      RichString_appendAscii(out, CRT_getAttrs(ZFS_OTHER), buffer);
   } else {
      RichString_writeAscii(out, CRT_getAttrs(METER_TEXT), " ");
      RichString_appendAscii(out, CRT_getAttrs(FAILED_SEARCH), "Unavailable");
   }
}
