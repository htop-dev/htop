...
void ProcessList_printHeader(ProcessList* this, RichString* header) {
   ...
   for (int i = 0; fields[i]; i++) {
      ...
      if (!field) field = "- ";
      if (this->settings->sortKey == fields[i])
...
// even though `Breadth-first search` approach may be more eficient from first perspetive,
// after comparision it may be not. As it's not save to not update the tree structure after getting deeper.
// If it would be save that approach would likely bring an advantage in performance.
//
// Each call of the function looks for a 'layer'. A 'layer' is a list of processes with the same deep.
// It sorts a list. Then it runs the function recursevely for each element of sorted list.
// And then it updates settings of processes.
//
// It relays on left_bound and right_bound as a optimization to cut the list size at the time it build a 'layer'.
//
// It use a temporaraly Hashtable `draftingTreeSet` because it's not save to traverse a tree
...
static void ProcessList_updateTreeSetLayer(ProcessList* this, unsigned int leftBound, unsigned int rightBound, unsigned int deep, unsigned int left, unsigned int right, unsigned int* index, unsigned int* treeIndex, int indent) {
   ...
   // Find all process on the same layer (process with the same deep value
   // and included in a range from left_bound to right_bound.
   //
   ...
   //
   // Thus, if present, set the right_bound to the last process on the layer
   if (Vector_size(layer) > 0) {
      Process* previous_process = (Process*)Vector_get(layer, Vector_size(layer)-1);
   ...
   for (int i = 0; i < size; i++) {
      ...
      // removing old index must be save
      // or it's considered an issue in the algorithm
...
static void ProcessList_updateTreeSet(ProcessList* this) {
   ...
   assert(Hashtable_count(this->displayTreeSet) == Vector_size(this->processes));

   ProcessList_updateTreeSetLayer(this, 0, vsize, 0, 0, vsize*2+1, &index, &tree_index, -1);
   assert(Hashtable_count(this->displayTreeSet) == 0);
   assert(Hashtable_count(this->draftingTreeSet) == vsize);
...
static long ProcessList_treeProcessCompare(const void* v1, const void* v2) {
   ...
   // return p1->tree_left - p2->tree_left;
   return (p1->tree_left < p2->tree_left ? -1 : 1);
}
