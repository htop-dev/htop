...
static void NetworkIOMeter_display(ATTR_UNUSED const Object* cast, RichString* out) {
   if (!hasData) {
      RichString_writeAscii(out, CRT_getAttrs(METER_VALUE_ERROR), "no data");
      return;
   }

   char buffer[64];

   RichString_writeAscii(out, CRT_getAttrs(METER_TEXT), "rx: ");
   Meter_humanUnit(buffer, cached_rxb_diff, sizeof(buffer));
   RichString_appendAscii(out, CRT_getAttrs(METER_VALUE_IOREAD), buffer);
   RichString_appendAscii(out, CRT_getAttrs(METER_VALUE_IOREAD), "iB/s");

   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " tx: ");
   Meter_humanUnit(buffer, cached_txb_diff, sizeof(buffer));
   RichString_appendAscii(out, CRT_getAttrs(METER_VALUE_IOWRITE), buffer);
   RichString_appendAscii(out, CRT_getAttrs(METER_VALUE_IOWRITE), "iB/s");

   xSnprintf(buffer, sizeof(buffer), " (%lu/%lu packets) ", cached_rxp_diff, cached_txp_diff);
   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), buffer);
}
