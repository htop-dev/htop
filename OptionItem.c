...
static void CheckItem_display(const Object* cast, RichString* out) {
   ...
   RichString_writeAscii(out, CRT_getAttrs(CHECK_BOX), "[");
   ...
   RichString_appendWide(out, CRT_getAttrs(CHECK_TEXT), this->super.text);
}
...
static void NumberItem_display(const Object* cast, RichString* out) {
   ...
   RichString_appendAscii(out, CRT_getAttrs(CHECK_MARK), buffer);
   RichString_appendAscii(out, CRT_getAttrs(CHECK_BOX), "]");
