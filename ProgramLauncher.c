...
static int ProgramLauncher_openAndCheckStat(const ProgramLauncher* this, ProgramFileRef* newFileRef) {
...
#ifdef O_REGULAR
      // O_REGULAR is supported in NetBSD.
      openFlags |= O_REGULAR;
#endif
      do {
   ...
   if (fd < 0) {
      int openFlags = O_NOCTTY | O_NONBLOCK;
#ifdef O_EXEC
      openFlags |= O_EXEC;
#else
...
static bool ProgramLauncher_isScriptFile(int fd) {
   ...
   char buf[2]; // This is intentionally not null terminated
   ssize_t len = 0;
   off_t start = 0;
   do {
      do {
         len = pread(fd, buf + start, sizeof(buf) - (size_t)start, start);
      } while (len < 0 && errno == EINTR);

      if (len <= 0) {
         // End of file (len == 0) or a read error other than EINTR
         return false;
      }
      start += len;
   } while (start < (off_t)sizeof(buf));

   assert(start == (off_t)sizeof(buf));
   return (buf[0] == '#' && buf[1] == '!');
}
