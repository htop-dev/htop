...
SYM_PRIVATE void LoadAverageMeter_updateValues(Meter* this, char* buffer, size_t size) {
...
SYM_PRIVATE void LoadAverageMeter_display(const Object* cast, RichString* out) {
...
SYM_PRIVATE void LoadMeter_updateValues(Meter* this, char* buffer, size_t size) {
   double five, fifteen;
...
SYM_PRIVATE void LoadMeter_display(const Object* cast, RichString* out) {
   const Meter* this = (const Meter*)cast;
   char buffer[20];
   xSnprintf(buffer, sizeof(buffer), "%.2f ", this->values[0]);
