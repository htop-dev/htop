...
void Platform_getBacktrace(Vector* frames, const BacktracePanel* panel, char** error) {
   ...
   const pid_t pid = Process_getPid(panel->process);
   ...
   do {
      ...
      BacktraceFrame* frame = BacktraceFrame_new(panel);
      frame->index = index;
      if (unw_get_proc_name(&cursor, procName, sizeof(procName), &offset) == 0) {
         ...
         if (ret < 0) {
            ...
            BacktraceFrame_delete((Object *)frame);
            break;
         }
         ...
         if (unw_get_elf_filename(&cursor, elfFileName, sizeof(elfFileName), &offsetElfFileName) == 0) {
            ...
            frame->objectName = xStrndup(splittedBySlash[n - 1], 2048);
