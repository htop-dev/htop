...
static void MemoryMeter_display(const Object* cast, RichString* out) {
   ...
   RichString_appendAscii(out, CRT_getAttrs(MEMORY_USED), buffer);
   Meter_humanUnit(buffer, this->values[1], sizeof(buffer));
   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " buffers:");
   RichString_appendAscii(out, CRT_getAttrs(MEMORY_BUFFERS_TEXT), buffer);
   Meter_humanUnit(buffer, this->values[2], sizeof(buffer));
   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " cache:");
   RichString_appendAscii(out, CRT_getAttrs(MEMORY_CACHE), buffer);
}
