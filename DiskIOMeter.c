...
static void DIskIOMeter_display(ATTR_UNUSED const Object* cast, RichString* out) {
   ...
   RichString_writeAscii(out, CRT_getAttrs(color), buffer);

   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " read: ");
   Meter_humanUnit(buffer, cached_read_diff, sizeof(buffer));
   RichString_appendAscii(out, CRT_getAttrs(METER_VALUE_IOREAD), buffer);

   RichString_appendAscii(out, CRT_getAttrs(METER_TEXT), " write: ");
