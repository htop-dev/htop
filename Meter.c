...
static inline size_t GraphMeterMode_valueCellIndex(unsigned int h, bool isPercentChart, int deltaExp, unsigned int y, unsigned int* scaleFactor, unsigned int* stride) {
   ...
   // This "cell index" function may be called for reading a cell or writing
   // one. When called for reading, the "scaleFactor" pointer must not be NULL.
...
cellIsEmpty:
   ...
   assert(itemIndex == 0 || itemIndex < maxItems);
