...
static void GraphMeterMode_recordNewValue(Meter* this, const GraphDrawContext* context) {
   ...
   if (!isPercentChart) {
      ...
      // "scaleExp" should never overflow when IEEE 754 (binary64)
      // floating point is used (DBL_MAX_EXP == 1024), but it wouldn't
      // hurt putting an assert here.
   ...
   if (sum <= 0.0) {
      // Empty record, no value or colors.
      return;
   }
