...
typedef struct GraphColorCell_ {
   /* Meter item number for the cell's color. Item numbers [1, 255]
      correspond to array indices [0, 254] respectively.
      0 means no color for the cell. */
   uint8_t itemNum;
   /* Bit field for how the character cell should be drawn. Each bit
      represents one eighth of the cell. LSB (bit 0) is the visual top
      eighth and MSB (bit 7) is the visual bottom eighth. See also the
      "printCellDetails" function code (with a diagram). */
   uint8_t details;
} GraphColorCell;
...
static void GraphMeterMode_recordNewValue(Meter* this, const GraphDrawContext* context) {
   ...
   if (!isPercentChart) {
      ...
      // Determine the scale and "total" that we need afterward. The
      // "total" is the "sum" value rounded up to a power of 2.
      int scaleExp = 0;
      (void)frexp(sum, &scaleExp);
      scaleExp = MAXIMUM(0, scaleExp);

      // It's safe to assume "scaleExp" never overflows when IEEE 754
      // (binary64) floating point is used. IEEE 754 always sets the
      // limit DBL_MAX_10_EXP == 308.
   ...
   // For a one-item meter, record the number of dots in the graph
   // data buffer and we are done.
   if (maxItems == 1) {
   ...
   // each cell and store them in a record, but clear the cells first,
   // which might contain data of the previous record.
